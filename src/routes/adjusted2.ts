...
export default function(p: MapPlot = new MapPlot()): MapPlot {
  ...
  const lady = plot.addRoute(platformReturn, 325, 19, 'Lady of the Lake', {label: false});
  ...
  const westPlatformWest = plot.addRoute(westPlatform, 270, 9, 'West Platform West Corner', {label: false, labelDot: false});
